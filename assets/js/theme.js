...
var bgPlayerInit = function bgPlayerInit() {
  ...
  var loadVideo = function loadVideo() {
    /* ------------------------------------------------------------------
       PRESENCE + CONSENT GATE.  See CLAUDE.md sec.18.

       This function used to inject the YouTube iframe API unconditionally.
       The [data-youtube-embed] check lived inside setupPlayer, which is the
       script's onload handler - so the request to Google had already gone
       out before anything asked whether the page contained a video. Every
       visitor, every page, video or not.

       First guard: nothing to play, nothing to load.

       Second guard: honours the visitor's consent choice. window.ccsConsent
       is defined by tail.htm, which this site now ships. A visitor who
       declined embedded media gets no YouTube request at all. The flag being
       absent means "not yet wired", NOT "denied", so the guard stays inert
       rather than blocking if tail.htm is ever swapped out.
       ------------------------------------------------------------------ */
    if (!youtubeEmbedElements.length) {
      return;
    }
    if (window.ccsConsent && !window.ccsConsent.embed) {
      return;
    }
...
var formInit = function formInit() {
  /* --------------------------------------------------------------------
     REMOVED - third-party exfiltration path.

     This bound to [data-form] and posted the contents of any such form
     through window.Email.send to a hardcoded smtp.mailtrap.io, using the
     theme's placeholder credentials ("Your User Name" / "Your Password").

     UNLIKE THE VALASPORTS COPY, THIS WAS NOT DORMANT HERE. This site does
     carry a [data-form] element - snippets/pb_mods/contact/theme/
     cntct_map_cntcfrm.html - so the listener was binding. It could never
     have delivered anything, because the credentials are placeholders, but
     it sat one filled-in field away from posting submitted form data to a
     third party.

     Forms on this site are handled server-side by CouchCMS (cms:form /
     cms:send_mail). Nothing here needs a client-side mailer. That contact
     theme carries onsubmit="return false;" and had no other submit path,
     so it was already non-functional; it needs a server-side handler
     before it can be used, not this.

     Left as a no-op rather than deleted so the docReady(formInit) call at
     the foot of this file stays valid.
     -------------------------------------------------------------------- */
};
...
/* ---------------------------------------------------------------------------
   GOOGLE MAPS REMOVED - the map feature is not used on this site.

   initMap() looked for [data-gmap] elements and, if window.google was present,
   built a Google Map or Street View panorama in one of ten bundled style
   themes. It also defaulted its marker icon to an image on maps.gstatic.com.

   IT WAS ALREADY DORMANT. Nothing on this site loads the Maps API - there is
   no maps.googleapis.com script anywhere - and the whole body sat behind a
   `window.google &&` guard, so it never ran. Removing it therefore changes
   nothing on screen. Dormant is not the same as harmless: it was a fully
   wired third-party surface one pasted script tag away from live, on a site
   whose consent framework promises no visitor reaches Google unasked.

   THE [data-gmap] MARKUP STILL EXISTS and is now inert placeholder divs:
       contact.php  x3   - theme demo layouts, coordinates in Paris, London
                           and Palo Alto, with a stock hotel marker
       snippets/pb_mods/contact/theme/cntct_map_cntcfrm.html  x1
   None of it is real content. It renders as an empty box with a min-height,
   exactly as it did before this deletion. Strip it when that contact page is
   built for real.

   IF A MAP IS EVER WANTED: it needs consent handling like every other third
   party here. See CLAUDE.md sec.18 - the pattern is data-src, never a live
   URL in the markup.
   --------------------------------------------------------------------------- */
function initMap() {}
