<?php
    ...
    class Color extends KUserDefinedField{
        ...
        function _render( $input_name, $input_id, $extra='', $dynamic_insertion=0 ){
            ...
            $value = htmlspecialchars( $color, ENT_QUOTES, K_CHARSET );

            $html = '<input type="color" name="' . $input_name . '[color]"  id="' . $input_id . '" value="' . $value . '" style="width:' . $this->field_width . ';';
            $html .= strlen( $this->field_height ) ? 'height:' . $this->field_height .';' : '';
            $html .= strlen( $alpha ) ? 'opacity:' . $alpha/255 .';' : '';
            $html .= '" oninput="getElementById(\'' . $input_id . "_alpha').style.backgroundImage = 'linear-gradient(to left, '+ this.value +' 0%, #fff 100%)';\"";
            $html .= $extra . '/>';

            $html = '<div style="border:thin solid #cecece; border-radius:5px;">' . $html . '</div>';
            ...
            if ( strlen( $alpha ) ){
                ...
                $html .= '<input type="range" name="' . $input_name . '[alpha]"  id="' . $input_id . '_alpha" value="' . $alpha . '" min="0" max="255" style="width:' . $this->field_width . '; border-radius: 3px;height: 7px; -webkit-appearance: none; background: linear-gradient(to left, '.$value.' 0%, #fff 100%);border: solid 1px #c0c0c0;" oninput="' . $script . '" />';
            }

            $html = '<div style="width:36%; min-width: 445px; border:1px solid #c0c0c0; border-radius:3px; padding:5px; ">' . $html . '</div>';
        ...
        function store_posted_changes( $post_val ){
            ...
            if( strlen($this->alpha) ){
                ...
                if( $post_val['alpha'] == 255 ){
                    ...
                    $post_val['alpha'] = str_pad( dechex( $post_val['alpha'] ), 2, '0', STR_PAD_LEFT);
                }
        ...
        function validate(){
            ...
            if( K_ADMIN_LANG != 'EN' && file_exists( str_replace( "\\", "/", dirname(__FILE__) ) . '/lang/' . K_ADMIN_LANG . '.php') ){
            ...
            if ( $this->validate != '0' ){
                ...
                if ( !preg_match( $pattern, $color ) ){
                    $this->err_msg = trim( $this->validator_msg ) ? $this->validator_msg : 'Wrong value';
                    return false;
                }
            }
