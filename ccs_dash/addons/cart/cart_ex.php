<?php
    ...
    class KCartEx extends KCart{
        ...
        function get_taxes(){
            ...
            // Get values from config
            $default_tax_percent = $this->get_config( 'tax_percent' );
            $tax_excludes_shipping = $this->get_config( 'tax_excludes_shipping' );
            if( !($tax_excludes_shipping==0 || $tax_excludes_shipping==1) ) $tax_excludes_shipping = 0;

            $get_tax_item = function( $taxable_amount, $tax_percent ){
                $tax_item = 0;
                if( is_numeric($tax_percent) && preg_match("/^[0-9.]+$/i", $tax_percent) && $tax_percent>0 ){
                    $tax_item = round( ($tax_percent/100)*$taxable_amount, 2 );
                }
                return $tax_item;
            };
            ...
            foreach( $this->items as $key=>$item ){
                ...
                if( $tax_class ){ // custom tax-class is set.. so use the indicated percentage for calculation
                    $tax_percent = $item['tax_rate'];
                    $taxable_amount = $item['orig_price'] * $item['quantity']; // use the original undiscounted price for calculation
                }
                else{ // go with the default tax calculation
                    $tax_class = 'Default';
                    $tax_percent = $default_tax_percent;
                    $taxable_amount = $item['line_total'];
                }

                if( $tax_item = $get_tax_item($taxable_amount, $tax_percent) ){
                    $tax_total += $tax_item;

                    if( array_key_exists($tax_class, $arr_taxes) ){
                        $tax_item += $arr_taxes[$tax_class];
                    }
                    $arr_taxes[$tax_class] = $tax_item;
                }
            }
            // total tax includes shipping?
            if( !$tax_excludes_shipping && ($tax_shipping = $get_tax_item($this->shipping, $default_tax_percent)) ){
                $tax_total += $tax_shipping;
                $arr_taxes['Shipping'] = $tax_shipping;
            }

            // save tax breakdown in the cart
            $this->set_custom_var( '__taxes', $arr_taxes );
    ...
    ///////////////////// @scratz - custom tax (https://www.couchcms.com/forum/viewtopic.php?f=3&t=13594)
    // cart - add selected tax value to item
    $FUNCS->add_event_listener( 'cart_alter_custom_fields', function(&$arr_custom_fields, &$pg, &$cart){
        global $DB;
        $fname = 'pp_tax_class';
        $fname2 = 'tax_rate';

        if( array_key_exists($fname, $pg->_fields) ){
            $f = $pg->_fields[$fname];
            if( $f->k_type=='relation' && $f->has=='one' && count($f->items_selected) ){
                $tax_id = $f->items_selected[0];

                if( $tax_id ){
                    $rs = $DB->select( K_TBL_PAGES, array('id', 'template_id'), "id = '".$DB->sanitize( $tax_id )."'" );
                    if( count($rs) ){
                        $rec = $rs[0];
                        $pg2 = new KWebpage( $rec['template_id'], $rec['id'] );
                        if( !$pg2->error ){
                            if( array_key_exists($fname2, $pg2->_fields) ){
                                $tax_rate = $pg2->_fields[$fname2]->get_data();
                                $arr_custom_fields['tax_class'] = ( $pg2->page_title!='' ) ? $pg2->page_title : $pg2->page_name;
                                $arr_custom_fields['tax_rate'] = $tax_rate;
                            }
                        }
                    }
                }
    ...
    // cart - add custom taxes as context variable
    $FUNCS->add_event_listener( 'add_render_vars', function(){
        global $CTX, $CART;

        if( is_object($CART) ){
            $taxes = $CART->get_custom_var( '__taxes' );
            if( !is_array($taxes) ){
                $taxes = array();
            }
