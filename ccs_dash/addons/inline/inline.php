        ...
        static function inline_handler( $params, $node ){
            ...
            extract( $FUNCS->get_named_vars(
                array(
                    ...
                    'height'=>'0',     /* -do- */
                    'skip'=>'',
                    'hide_fields'=>'', /* NEW: Accepts hidden fields */
                ),
                $params)
            );
            ...
            $skip = trim( $skip );
            $hide_fields = trim( $hide_fields ); /* NEW: Sanitize parameter */
            ...
            if( $node->name!='inline_link' && strlen($hide_fields) ){ $url = $url . '&hide_fields='.urlencode($hide_fields); } /* NEW: Pass to backend */
