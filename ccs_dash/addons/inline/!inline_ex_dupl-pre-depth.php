<?php
    ...
    class InlineEx extends KBaseAdmin{
        ...
        var $hide_fields = array(); /* NEW: Initialize array */
        ...
        function edit_action(){
            ...
            // get fields to hide
            $this->hide_fields = array_filter( array_map("trim", explode('|', (isset($_GET['hide_fields'])) ? $_GET['hide_fields'] : '')) ); /* NEW: Parse from URL */
        ...
        function _set_default_fields( &$arr_fields ){
            global $FUNCS, $PAGE, $AUTH; /* BUGFIX: Added $AUTH to global scope */
            ...
            foreach( $this->arr_fields as $k=>$f ){
                if( !is_object($f) ){
                    ...
                    $def = array(
                        ...
                        'no_wrapper'=>0,
                        'skip'=>0,
                        'hide'=>( ($f->system && $f->hidden) || ($f->deleted && $AUTH->user->access_level < K_ACCESS_LEVEL_SUPER_ADMIN) || $f->no_render || in_array($f->name, $this->hide_fields) ) ? 1 : 0, /* MODIFIED: Added in_array check */
                        'required'=>$f->required,
